...
void PlotDataFit(TH1D hist_sum0, TH1D hist_bkg0, TH1D hist_sum_SM0, TGraphErrors datahist0, TH1D hist_unc, std::string input_channel, std::string fOutputFolder)
{
	...
	TH1D         *hist_mcUnc  = (TH1D*) hist_sum -> Clone("");
	...
	for(int iBin = 1; iBin <= nBins; ++iBin){
	  ...
	  double errMC     = hist_sum -> GetBinError(iBin) / hist_sum -> GetBinContent(iBin);
	  ...
	  //cout << iBin << "\t" << val << "\t" << y << "\t" << err << "\t" << ye << "\t" << ratio << "\t" << ratio_unc << endl;

	  hist_ratio -> SetBinContent(iBin, ratio);
	  hist_ratio -> SetBinError(iBin, ratio_unc);
	  hist_mcUnc -> SetBinContent(iBin, 1);
	  hist_mcUnc -> SetBinError(iBin, errMC);
	...
	if(input_channel == "el_mu")
	  ...
	  c0 = new TCanvas("c0","c0",  600, 600);
		...
		c0 = new TCanvas("c0","c0",  2000, 1000);
	else
	  c0 = new TCanvas("c0","c0", 1800, 1000);
	...
	//double max = HistSumUnc.GetBinContent(HistSumUnc.GetMaximumBin())*1.65;
	double max = hist_sum->GetBinContent(hist_sum->GetMaximumBin())*1.75;

	hist_sum->GetXaxis()->SetTitle("cos #theta*");
	hist_sum->GetYaxis()->SetTitle("Events / 0.1333");
	hist_sum->GetXaxis()->SetTitleSize(0.055);
	hist_sum->GetYaxis()->SetTitleSize(0.062);
	hist_sum->GetYaxis()->SetTitleOffset(0.85);
	hist_sum->GetYaxis()->SetTitleOffset(1.);

	if(input_channel == "el_BTag" || input_channel == "mu_BTag"){
	  hist_sum->GetYaxis()->SetTitleOffset(0.80);
	  hist_sum->GetYaxis()->SetLabelSize(0.060);
	}
	if(input_channel == "el" || input_channel == "mu"){
          hist_sum->GetYaxis()->SetTitleOffset(1.025);
          hist_sum->GetYaxis()->SetLabelSize(0.050);
        }
	else{
	  hist_sum->GetYaxis()->SetTitleOffset(0.85);
	  hist_sum->GetYaxis()->SetLabelSize(0.065);
	}
	...
	HistSumUnc.SetMarkerStyle(1);
	HistSumUnc.SetMarkerColor(kGreen);
	HistSumUnc.SetLineColor(kGreen);
	HistSumUnc.SetFillColor(kGreen);
	...
	lBen.SetNDC(1);
	lBen.SetTextFont(62);
	lBen.SetTextSize(0.040);
	if (input_channel == "el")    l.DrawLatex(0.160,0.81, "e+jets channel");
	if (input_channel == "mu")    l.DrawLatex(0.160,0.81, "#mu+jets channel");
	if (input_channel == "el_mu"){
	  l.DrawLatex(0.140, 0.780,  "Leptonic analyser");
	  lBen.DrawLatex(0.220, 0.58, "e+jets (#geq 2 b-tags)");
	  lBen.DrawLatex(0.65, 0.58, "#mu+jets (#geq 2 b-tags)");
	}
	if (input_channel == "el_BTag"){
	  ...
	  l.DrawLatex(0.760, 0.125, "- #geq 2 B-tag -");
        }
    ...
    if (input_channel == "el_mu_bTag"){
	  ...
	  lBen.DrawLatex(0.18, 0.58, "e+jets (1 b-tag)");
	  lBen.DrawLatex(0.38, 0.58, "#mu+jets (1 b-tag)");
	  lBen.DrawLatex(0.59, 0.58, "e+jets (#geq 2 b-tags)");
	  lBen.DrawLatex(0.78, 0.58, "#mu+jets (#geq 2 b-tags)");

          /*l.DrawLatex(0.140, 0.120, "- e+jets(1excl.) -");
	  l.DrawLatex(0.350, 0.120, "- #mu+jets(1excl.) -");
	  l.DrawLatex(0.560, 0.120, "- e+jets(2incl.) -");
	  l.DrawLatex(0.760, 0.120, "- #mu+jets(2incl.) -");*/
        }

    if (input_channel == "el_mu_lephad_bTag"){
          l.DrawLatex(0.140, 0.795,  "Combined channel");
          l.DrawLatex(0.150, 0.085, "- e+jets(1excl.) -");
	  	  l.DrawLatex(0.360, 0.085, "- e+jets(2incl.) -");
	  	  l.DrawLatex(0.570, 0.085, "- #mu+jets(1excl.) -");
	  	  l.DrawLatex(0.770, 0.085, "- #mu+jets(2incl.) -");
        }

	//set labels....
	std::string fLumiLabel = "#int L dt = 20.2 fb^{-1}, #sqrt{s} = 8 TeV)";
	TLatex l1;
	l1.SetTextAlign(9);
	l1.SetTextSize(0.04);
	l1.SetNDC();
	l1.DrawLatex(0.14, 0.680, fLumiLabel.c_str());
	l1.Draw("same");
	...
	/*
	  ...
	  l.SetTextSize(0.040);
          ...
          l.DrawLatex(0.140, 0.730, ("#chi^{2}/ndf = "+oss.str()).c_str());
	...
	*/
	...
	//legend0->AddEntry(&HistSumUnc,  "Best Fit Unc. (Stat.)",   "f");
	...
	TPad *pad2 = new TPad("pad2","pad2",0,0.00,1,0.225);
	pad2->SetTopMargin(0);
	...
	hist_ratio->SetMinimum(0.94);
	hist_ratio->SetMaximum(1.06);
	...
	hist_mcUnc->SetMarkerColor(kBlack);
	hist_mcUnc->SetFillStyle(3004);
	hist_mcUnc->SetFillColor(kBlack);
	hist_mcUnc->SetStats(kFALSE);
	hist_mcUnc->SetLineColor(0);
	hist_mcUnc->SetLineWidth(1);
	hist_mcUnc->Draw("E2same");
	...
	norm1 -> SetLineStyle(2);
	norm1 -> SetLineWidth(.9);
        norm1 -> Draw("Same");
...
void PlotDataFitSimple(TH1D hist_sum, TH1D hist_bkg, TH1D hist_sum_SM, TGraphErrors datahist, std::string input_channel, std::string fOutputFolder)
{
  //	SetAtlasStyle();
	...
	if (input_channel == "el_mu") l.DrawLatex(0.160,0.82, "Leptonic Analyser");
...
void PlotDataFit(TH1D hist_sum0, TH1D hist_bkg0, TH1D hist_sum_SM0, TH1D datahist0, TGraphAsymmErrors* hist_unc, TH1D hist_ratio0, std::string input_channel, std::string fOutputFolder)
{
	...
	hist_sum->GetYaxis()->SetTitle("#scale[1.23]{Events / 0.1333}");
	...
	//set labels....
	std::string fLumiLabel = "#int L dt = 20.2 fb^{-1}";
	TLatex l1;
	l1.SetTextAlign(9);
	l1.SetTextSize(0.04);
	l1.SetNDC();
	l1.DrawLatex(0.23, 0.730, fLumiLabel.c_str());
	l1.Draw("same");
	...
	hist_ratio->GetYaxis()->SetTitleOffset(1.03);
	hist_ratio->GetYaxis()->SetLabelSize(0.048);
	...
	std::string printtitle2 = fOutputFolder+"/Datafit_" + input_channel + ".gif";
	std::string printtitle3 = fOutputFolder+"/Datafit_" + input_channel + ".eps";
	std::string printtitle4 = fOutputFolder+"/Datafit_" + input_channel + ".pdf";
